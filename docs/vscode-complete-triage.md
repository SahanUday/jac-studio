...
| debug | Scoped | Phase 4 — DAP client (moved from the old Phase 5 on 2026-08-31, same native-infrastructure reasoning as the LSP client — see `roadmap.md`) |
...
| testing | Tracked | Gap analysis Tier 2 — Phase 6+, needs extension manifests (the DAP client itself now lands earlier, Phase 4) |
...
| chat, inlineChat, interactive, mcp, welcomeAgentSessions | Tracked, **partially Scoped (2026-08-31)** | Gap analysis Tier 2.5 — 442,661 lines, deliberately not ported. A generic arbitrary-chat-extension mechanism stays Tracked/deferred (same trust-boundary work as the extension-system Phase B/C track), but a small named set of native integrations (GitHub Copilot, OpenCode, Claude Code) is now Scoped into `roadmap.md`'s Phase 5, alongside `by llm()`/`sem` as the native first-class path — see `architecture.md`'s "AI coding tool integrations" section. |
| **authentication** | **New — Tracked** | 1,356 lines. Auth-provider broker letting extensions share OAuth tokens (e.g. GitHub sign-in used by multiple extensions at once) rather than each prompting separately. Needed the moment any extension needs to authenticate against an external service. Add to Phase 6 alongside the extension manifest work — a natural pair with `encryption` below. Phase 5's Copilot/OpenCode/Claude Code integrations will need *some* auth handling earlier than this — worth checking there whether their needs are simple enough to defer reusing this broker, or whether a lightweight piece of it needs pulling forward. |
...
| **editSessions** | **New (v3) — Tracked** | ~110KB across 7 files. "Continue Working On" — syncs uncommitted changes across devices via a Microsoft-account-backed cloud store, so a user can pick up mid-edit on another machine. Genuinely missed by v1/v2 (never checked against the live upstream tree). Pairs with the already-Tracked `userDataSync`/`userDataProfile` and depends on the same `authentication` broker staged for Phase 6 — group with those, no earlier. |
| **update** | **New (v3) — Tracked, partially Scoped** | ~120KB across 8 files: in-app update UI (release-notes viewer, "Restart to Update" notification, title-bar update indicator) — distinct from `roadmap.md` Phase 8's "auto-update feed" bullet (re-numbered 2026-08-31, was Phase 7), which is the *packaging/build* side (the feed itself, code signing). Phase 8's bullet covers shipping updates; this is the in-app UI *telling the user* one's available — add as an explicit Phase 8 sub-bullet so it isn't assumed to fall out of the feed for free. |
...
|---|---|---|
| **activitybar** | **Done (Phase 3), Search/SCM views land Phase 4** | The icon rail down the left edge (Explorer/Search/SCM/Debug/Extensions, sidebar toggle). `architecture.md`'s primitive-mapping table previously folded this into one "Activity bar + sidebar → `Sidebar`" row — that's wrong: `activitybarPart.ts` is its own module upstream, distinct from `sidebar`, because it's a multi-view *switcher*, not a container. Built 2026-08-28 (`src/workbench/activity_bar/activity_bar.jac`) with exactly one real entry (Explorer) — the switching mechanism itself is real and end-to-end, so Phase 4's Search/SCM views are a one-entry-plus-one-branch addition, not new plumbing under time pressure. |
| **titlebar** | **Done (Phase 3)** | The custom title bar. Built 2026-08-28 (`src/workbench/title_bar/title_bar.jac`) as chrome + a centered Command Center search box that opens the command palette — deliberately without window controls (a web app, not a native desktop host yet) or a menu bar (separate, materially larger scope this bullet never named). |
| **auxiliarybar** | **New — Tracked** | The secondary/right-side dockable panel (upstream uses it for Chat/Copilot and any view a user drags there). Generic dockable-panel infra, useful independent of chat (which stays Tier 2.5/excluded per the `chat` row above) — e.g. as a second home for Search results or SCM. No phase assigned; revisit once Phase 4/6 has more than one panel-worthy view competing for space. |
...
**Language-intelligence cluster — Phase 4, consuming provider data from the native `jac lsp` client
(promoted from an open-ended "Phase 4+" to concretely Phase 4 on 2026-08-31, see `roadmap.md`)**
(this is what the original v1 clustering was gesturing at, correctly for this subset):
`codeAction`, `codelens`,
...
`by llm()` may give it a different natural shape than upstream's. **Note (2026-08-31)**: this is
also the most likely concrete home for the Phase 5 GitHub Copilot integration's actual UI surface
(ghost-text completions), since that's Copilot's own primary interaction mode in real VS Code too
— worth designing this editor-level widget with that integration in mind once Phase 5 is scoped in
detail, rather than assuming it's `by llm()`-only.
...
- **`extensions/`** — the 106 built-in extensions (language grammars, `git`, `github-
  ...
  Phase 4/6 extension system and Phase 3 syntax-highlighting exist, each of these becomes "an
