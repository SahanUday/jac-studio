...
| **themes** | **Split: Scoped (Phase 3) + open question (Phase 4/5)** | 1,631 lines. Upstream's installable, JSON-based color-theme/icon-theme extension model. Jac already has its own theming primitive (`jac retheme`, OKLCH-based, per `jac-shadcn-components.md`) — a *different* mechanism. Two separate questions, previously conflated: (1) **default visual identity** — should jac-studio's out-of-the-box look match VS Code's own Dark+/Light+ palette and Codicons, regardless of any extension system? **Decided 2026-08-28** — yes, via native `jac retheme` tokens, scoped into Phase 3 (see `roadmap.md`). (2) **installable third-party `.vsix` theme-extension compatibility** — still a genuine open question, not decided; worth resolving once Phase 4/5 extensions exist to actually contribute a theme — see `architecture.md`'s open questions list. |
